//This REALLY should be triggered in an alarm 
if ( alarm[0] <= 0 )
{
  scp_controlller_grid_move_enemy();
  alarm[0] = 20;
}
//Select player
if ( mouse_check_button_pressed(mb_left) )
{
    selected_object = instance_position(mouse_x,mouse_y,obj_grid_box)
}

//To move the player we move the player's obj_grid_box.
//The actual player object follows it's grid box on the map.
//This makes it so we can have tween animations and still collide with projectiles etc.

//Move the player around
//IF the selected object is a player unit's grid box and not set to the default value of -1
if ( instance_exists( selected_object ) && selected_object != -1  )
{
    if ( object_is_ancestor(selected_object.parent.object_index,obj_player_parent)  ) 
    {
        ...
        if ( mouse_check_button(mb_left) )
        {
            ...
            if ( move_rot >= 0)
            {
                ...
                {
                   tmp_other = instance_place(x + move_x, y + move_y,obj_grid_box)
                }
                ...
                if ( tmp_other )
                {
                  tmp_swap = object_is_ancestor(tmp_other.parent.object_index, obj_player_parent )
                }
                else
                {
                  tmp_swap = false
        ...
        if ( mouse_check_button_released(mb_left) )
        {
            ...
            if ( !tmp_other  || tmp_swap )
            {
                ...
                //Move player
                //This SHOULD be how many pixels per step to move divided by 
                //grid_size_x or grid_size_y depending on direction moving
                selected_object.alarm[0] = grid_size_x / move_speed;
                selected_object.x += move_x;
                selected_object.y += move_y;
                //Create a pause before the player can be moved again...0] = grid_size_x / move_speed; //selected_object.move_ready;
            }
